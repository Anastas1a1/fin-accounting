...
async def main_menu(call: CallbackQuery):
    ...
    if call.data == "btn:NameMenu:Choice":
        names_list = tbls.get_names(call.message.chat.id)
        if names_list == 0:
...
async def choice_cur(call: CallbackQuery, state: FSMContext):
    ...
    if call.data == "btn:Rate:New":
        ...
        async with state.proxy() as data:
            data['rate'] = tables.parse_rate(data['cur'])
            if data['rate'] == 0:
...
async def choice_cur(call: CallbackQuery, state: FSMContext):
    ...
    # tables.add_transaction(call.message.chat.id, transaction)
    ...
    await bot.send_message(
        ...
        md.text(
            ...
            md.text('   Currency Rate: ', data['rate']),
            sep='\n',
        ),
...
async def other_cur(message: types.Message, state: FSMContext):
    ...
    # tables.add_transaction(message.chat.id, transaction)
